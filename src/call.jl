...
using Juliette, HypothesisTests
...
function call(genome_path, bam_paths...; alt_reads=3, alt_frac=0.1,
	...
	max_alleles = 100
	for line in eachline(pipeline(`samtools mpileup -d 1000000 -A -x -R -sB -q0 $(mpileup_options) -f $(genome_path) $(bam_paths)`, `$(spileup) $(alt_reads) $(min_mapq)`))
...
call(genome_path, bam_paths, alt_reads, alt_frac, min_mapq, region)
